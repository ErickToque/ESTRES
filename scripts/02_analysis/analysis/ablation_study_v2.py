...
- Probando diferentes modalidades: EDA, HRV, TEMP, ACC y combinaciones
"""
import numpy as np
import pandas as pd
...
def extract_temp_features(temp_df):
    """Extrae características de temperatura"""
    if temp_df is None:
        return np.zeros(3)
...
def extract_acc_features(acc_file_path):
    ...
    try:
        ...
        # ACC tiene formato: timestamp_x,timestamp_y,timestamp_z (línea 0)
        # luego frecuencias (línea 1), luego datos x,y,z
        if len(lines) < 3:
            return np.zeros(6)
...
def load_all_data():
    """Carga todos los datos con todas las modalidades (mismo filtro que ventanas)"""
    ...
    for p in participants:
        for exam in exams:
            ...
            # ========== EDA (mismo proceso que en ventanas) ==========
...
def evaluate_model(data, feature_keys, model_name):
    """Evalúa modelo con LOSO (misma metodología que ventanas)"""
    if len(data) < 3:
    ...
    # Eliminar features constantes (como en ventanas)
...
# Definir configuraciones (mismo orden que en ventanas)
configs = [
